...
export class LoginPage implements OnInit {
  ...
  localStorageSvc = inject(LocalStorageService); // Servicio de localStorage
  ...
  async getUserInfo(uid: string) {
    if (this.form.valid) {
      ...
      this.firebaseSvc.getDocument(path).then((user: User) => {
        if (user) {
          this.utilsSvc.saveInLocalStorage('user', user); // Guardar en localStorage usando Utils
          this.localStorageSvc.setItem('user_backup', user); // Guardar como respaldo en localStorage directamente
